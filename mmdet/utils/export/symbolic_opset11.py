...
# Original source file is available here (Copyright (c) 2020 Facebook):
# https://github.com/pytorch/pytorch/blob/master/torch/onnx/symbolic_opset11.py
#
...
def clamp(g, self, min, max):
    ...
    def _cast_if_not_none(tensor, dtype):
        if tensor is not None and not _is_none(tensor):
...
def arange(g, *args):
    ...
    def _arange_cast_helper(g, end, start=None, step=None, dtype=None):
        ...
        if sym_help._is_value(dtype) and _is_none(dtype):
            if _is_all_integral([start, end, step]):
                type = sym_help.scalar_type_to_pytorch_type.index(torch.int64)
            else:
                type = sym_help.scalar_type_to_pytorch_type.index(torch.get_default_dtype())
        else:
            type = dtype

        start = g.op("Cast", start, to_i=sym_help.scalar_type_to_onnx[type]) if start else None
        end = g.op("Cast", end, to_i=sym_help.scalar_type_to_onnx[type]) if end else None
        step = g.op("Cast", step, to_i=sym_help.scalar_type_to_onnx[type]) if step else None
        return type, end, start, step

    if len(args) == 5:
        # aten::arange(Scalar end, ScalarType dtype, Layout, Device, bool pin_memory)
        dtype = _get_arange_dtype(args[1])
        type, end, start, step = _arange_cast_helper(g, end=args[0], dtype=dtype)
        ...
        type, end, start, step = _arange_cast_helper(g, start=args[0], end=args[1], dtype=dtype)
        delta_default = g.op("Constant", value_t=torch.tensor(1, dtype=sym_help.scalar_type_to_pytorch_type[type]))
        arange_tensor = g.op("Range", start, end, delta_default)
    elif len(args) == 7:
        # aten::arange(Scalar start, Scalar end, Scalar step, ScalarType dtype, Layout, Device, bool pin_memory)
        dtype = _get_arange_dtype(args[3])
        type, end, start, step = _arange_cast_helper(g, start=args[0], end=args[1], step=args[2], dtype=dtype)
...
def size(g, self, dim):
    ...
    return _size_helper(g, self, dim)
